                        ...
                        <div class="flex flex-col md:flex-row gap-lg">
                            <div class="flex-1 space-y-md">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div>
                                        <label class="block font-label-sm text-on-surface-variant uppercase mb-xs">Agent type</label>
                                        <select name="mode" class="w-full px-4 py-base bg-white border border-slate-200 rounded-lg">
                                            <option value="chatbot" {{ ($bot->mode ?? 'chatbot') === 'chatbot' ? 'selected' : '' }}>Website chatbot</option>
                                            <option value="agent" {{ ($bot->mode ?? '') === 'agent' ? 'selected' : '' }}>Work agent</option>
                                        </select>
                                    </div>
                                    <div>
                                        <label class="block font-label-sm text-on-surface-variant uppercase mb-xs">Autonomy</label>
                                        <select name="autonomy" class="w-full px-4 py-base bg-white border border-slate-200 rounded-lg">
                                            <option value="draft" {{ ($bot->autonomy ?? '') === 'draft' ? 'selected' : '' }}>Draft only</option>
                                            <option value="approve" {{ ($bot->autonomy ?? 'approve') === 'approve' ? 'selected' : '' }}>Plan then approve</option>
                                            <option value="auto" {{ ($bot->autonomy ?? '') === 'auto' ? 'selected' : '' }}>Auto-run safe work</option>
                                        </select>
                                    </div>
                                </div>
                                <div>
                                    <label class="block font-label-sm text-on-surface-variant uppercase mb-xs">Primary goal</label>
                                    <input name="goal" class="w-full px-4 py-base bg-white border border-slate-200 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none" placeholder="What should this agent achieve?" value="{{ $bot->goal ?? '' }}" />
                                    <p class="text-[11px] text-slate-500 mt-1">Work agents create a reviewable plan for every task before producing the deliverable.</p>
                                </div>
                            </div>
                    ...
                    <section class="bg-white p-lg border border-slate-200 rounded-xl shadow-sm">
                        <div class="flex items-center justify-between mb-md">
                            ...
                            <div class="flex items-center gap-2">
                                <span
                                    class="bg-secondary-container text-on-secondary-container px-2 py-1 rounded text-[10px] font-bold uppercase tracking-wider">Groq</span>
                            </div>
                        </div>
                        <div class="relative">
                ...
                configForm.addEventListener('submit', function(e) {
                    ...
                    const jsonObject = {
                        ...
                        "first_message": document.querySelector('input[name="first_message"]')?.value ||
                            "Hello!",
                        "mode": document.querySelector('select[name="mode"]')?.value || "chatbot",
                        "goal": document.querySelector('input[name="goal"]')?.value || "",
                        "autonomy": document.querySelector('select[name="autonomy"]')?.value || "approve",
                        "owner_id": "{{ auth()->id() ?? 'default_owner' }}"
                    };
                    ...
                    fetch("/bots/update/" + botId, {
                        ...
                        .then(({ ok, data }) => {
                            if (ok && (data.status === 'success' || data.bot_id)) {
                                // Newly created bots come back with a bot_id — take the
                                // user straight into that bot's config instead of always
                                // bouncing to the dashboard.
                                const newBotId = data.bot_id;
                                if (botId === 'new' && newBotId) {
                                    window.location.href = '/bot-config/' + newBotId;
                                } else {
                                    window.location.href = '/';
                                }
                            } else {
                                alert('Backend Error: ' + (data.message || JSON.stringify(data)));
                            }
                        })
                        .catch(error => {
