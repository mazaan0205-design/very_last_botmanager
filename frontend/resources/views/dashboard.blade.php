    ...
    <aside id="sidebar"
        ...
        <nav class="flex-1 flex flex-col gap-1">
            <a href="{{ route('dashboard') }}"
                class="flex items-center gap-3 px-4 py-2 rounded-lg transition-all {{ request()->routeIs('dashboard') ? 'text-indigo-600 bg-indigo-50 font-medium' : 'text-slate-600 hover:bg-slate-200/50' }}">
                <span class="material-symbols-outlined">dashboard</span> Dashboard
            </a>
            <a href="{{ route('bot-config', ['id' => $bot->id ?? 'new']) }}"
                class="flex items-center gap-3 px-4 py-2 rounded-lg transition-all {{ request()->routeIs('bot-config') ? 'text-indigo-600 bg-indigo-50 font-medium' : 'text-slate-600 hover:bg-slate-200/50' }}">
                <span class="material-symbols-outlined">settings</span> Configuration
            </a>
            <a href="{{ route('knowledge', ['id' => $bot->id ?? 'new']) }}"
                class="flex items-center gap-3 px-4 py-2 rounded-lg transition-all {{ request()->routeIs('knowledge') ? 'text-indigo-600 bg-indigo-50 font-medium' : 'text-slate-600 hover:bg-slate-200/50' }}">
                <span class="material-symbols-outlined">database</span> Knowledge Base
            </a>
            <a href="{{ route('preview', ['id' => $bot->id ?? 'new']) }}"
                class="flex items-center gap-3 px-4 py-2 rounded-lg transition-all {{ request()->routeIs('preview') ? 'text-indigo-600 bg-indigo-50 font-medium' : 'text-slate-600 hover:bg-slate-200/50' }}">
...
<main class="w-full pt-16 min-h-screen">
    <div class="p-4 md:p-[40px]">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <div class="flex items-center gap-1.5 text-xs text-slate-500">
                                    <span
                                        class="w-2 h-2 rounded-full {{ ($bot->status ?? 'offline') == 'online' ? 'bg-green-500' : 'bg-slate-400' }}"></span>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach ([['id' => 'lead_capture', 'name' => 'Smart Lead Capture', 'icon' => 'person_add', 'desc' => 'Autonomously collects names, emails, and phones.'], ['id' => 'live_scheduler', 'name' => 'Live Scheduler', 'icon' => 'calendar_month', 'desc' => 'Books meetings directly into your calendar.'], ['id' => 'faq_resolver', 'name' => 'Instant FAQ Resolver', 'icon' => 'help', 'desc' => 'Scans your knowledge base for instant answers.']] as $ext)
...
<script>
    ...
    function openEmbedModal(botId) {
        ...
        // Using window.location.origin to dynamically get the current URL base
        const baseUrl = window.location.origin;
        const snippet = `<script>
  window.BotManagerConfig = { botId: "${botId}", apiUrl: "${baseUrl}" };
<\/script>
<script src="${baseUrl}/embed/widget.js" defer><\/script>`;
    ...
    // Ensure this variable exists at the top of your script block
const BASE_URL = "http://127.0.0.1:8001";

async function toggleExtension(extensionId, button) {
    const card = button.closest('.bg-white'); // The main card container
    ...
    try {
        // Try sending the data like this, ensuring the key matches what the backend expects
await fetch(`${BASE_URL}/bots/${botId}/extensions/${extensionId}/toggle`, {
    method: 'POST',
    headers: {
        'Content-Type': 'application/json'
    },
    body: JSON.stringify({
        // Try 'enabled' or 'active'. Check your backend code for the expected field name!
        enabled: true
    })
});
        ...
        alert("Could not connect to backend. Please check that your backend server is running on port 8001.");
        button.innerText = originalText;
        button.disabled = false;
    }
}
...
async function toggleExtension(extensionId, button) {
    ...
    try {
        const response = await fetch(`${BASE_URL}/bots/${botId}/extensions/${extensionId}/toggle`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json' // Tells Laravel to return JSON errors
            },
            // Adding a payload body.
            // NOTE: Change 'enabled' to whatever field your backend expects (e.g., 'active', 'status', etc.)
            body: JSON.stringify({
                enabled: true
            })
        });
        ...
        if (response.ok) {
            ...
            setTimeout(() => {
                button.innerText = "Toggle Extension";
                button.classList.remove('bg-green-600');
                button.classList.add('bg-indigo-600');
                button.disabled = false;
            }, 2000);
        } else {
            // THIS WILL SHOW YOU THE EXACT VALIDATION ERROR
